      ...
      this.$refs.pwdModelRef.validate((value) => {
        if (value) {
          if (this.pwdModel.newPwd == this.pwdModel.secondayPwd) {
            this.$confirm("This operation will change the password,Continue?", "Attention", {
              ...
              .then(() => {
                this.$http
                  .post(this.base_url + "api/t/pwd/update", this.pwdModel)
                  .then((res) => {
                    if (res.data.code == 200) {
                      ...
                      _than.router.go(0);// refresh the page
                    }else{
                       ElMessage({
                        type: "error",
